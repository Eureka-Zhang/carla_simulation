...
用法:
    python RightBack.py [--display 2] [--width 250] [--height 200]
                        [--trapezoid x1,y1,...,x4,y4] [--no-trapezoid]
                        [--no-transparent] [--colorkey R,G,B]
...
# 相机配置
CAMERA_CONFIG = {
    'name': 'Right Rearview Mirror',
    # 默认窗口尺寸；镜面比例同步以避免缩放伪影
    'width': 300,
    'height': 290,
    'fov': 100,
    'mirror_aspect': 300/280,
    # 相机内部采样分辨率（必须为 4 的倍数，宽度不是 4 的倍数会出现斜条纹）
    'sensor_width': 512,
    'sensor_height': 512,
    # 相机位置：右后视镜视角
    'location': carla.Location(x=0.4, y=1, z=1.1),
    'rotation': carla.Rotation(pitch=-3, yaw=200),
    'flip': True,  # 后视镜需要翻转
    # 梯形遮罩: 4 个角点，按顺序 [左上, 右上, 右下, 左下]，数值为占窗口 (宽, 高) 的比例 0~1
    # 默认模拟右后视镜外窄内宽的轮廓（左右镜像于左后视镜）；设为 None 可关闭
    'trapezoid_corners': [
        (0.45, 0.00),  # 左上
        (0.9, 0.05),  # 右上
        (1.0, 0.70),  # 右下
...
def main():
    argparser = argparse.ArgumentParser(description='右后视镜相机')
    ...
    argparser.add_argument('--mirror-aspect', default=CAMERA_CONFIG['mirror_aspect'], type=float,
                          help='后视镜画幅宽高比（默认 1.25）')
    argparser.add_argument('--trapezoid', default=None, type=str,
                          help='梯形遮罩 4 个角点，按 [左上, 右上, 右下, 左下] 顺序的 x,y 比例，共 8 个 0~1 数字，逗号分隔。例: 0.07,0,0.65,0.2,0.75,0.8,0.07,1')
    ...
    argparser.add_argument('--pos-y', default=650, type=int, help='窗口Y位置（相对该显示器左上角）')
